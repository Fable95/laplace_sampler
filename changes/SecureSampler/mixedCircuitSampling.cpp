 ...
 */
 ...
 template<class T>
 vector<T> transform_share(vector<typename T::bit_type>& bit_vector, MixedProtocolSet<T>& set, 
                             MixedProtocolSetup<T>& setup, Player& P, vector<pair<T, typename T::bit_type>>& dabits){
     auto& bit_output = set.binary.output;
     auto& prep = set.preprocessing;
     int size = bit_vector.size();
     int dabit_size = dabits.size();
     bool dabit_necessary = dabit_size < size;
     bit_output.init_open(P, size);
     pair<T, typename T::bit_type> dabit;
 
     if(dabit_necessary){
         dabits.resize(size);
         for (int i = dabit_size; i < size; i++){
             dabit = pair<T, typename T::bit_type>();
             prep.get_dabit(dabit.first, dabit.second);
             dabits.at(i) = dabit;
             dabits_count++;
         }
         std::cout << "dabit count after creation " << dabits.size() << std::endl;
     }
     for (int i = 0; i < size; i++){
 ...
 void run(char** argv)
 {
     ...
     // vector<pair<T, typename T::bit_type>> dabits;
